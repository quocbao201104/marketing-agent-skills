...
Do not add a separate role audit to normal execution. Only when a candidate audience-facing decision would materially change what retained state functions as — for example, turning a specification, proof item, objection, barrier, commercial condition, or representation requirement into a dominant proposition, angle, headline, subject, or cross-surface premise — verify that the current reader state, touchpoint job, evidence, or adopted decision supports that role change before committing it. Local foregrounding without a role change may proceed; supported promotion updates the resolved state, while unsupported promotion returns to the last supported role and repairs only the affected downstream choice. Repetition triggers the same check only when it would turn local emphasis into a broader organizing premise.
...
- **Establish identity inputs before choosing identifying cues.** Knowing a project's name and functions does not by itself establish its audience, intended recognition, or deployment requirements. For a new or changed identity, use retained context and available project documentation/assets to establish the inputs that could change the visual choice; distinguish established facts from delegated choices and candidate assumptions. When an identifying-cue decision remains open, use the relevant `brand-identity` guidance before selecting its visual realization. Resolve material missing inputs through the uncertainty policy; do not silently choose a metaphor or assume a usage context to fill them. Provisional sketches may make an open choice inspectable, but do not establish strategy or deployment fitness. Validate selected assets in the required uses before claiming completion. Sufficient retained inputs need no repeated intake or extra approval; an approved identity needing only mechanical production stays on the direct path.
